...
void sendBytes(int len) { // not in a message frame! Trash!
    for(uint8_t i = 0; i < len / 2; i++) seningUart.putDataByte(i);
    for(uint8_t i = 0; i < len / 2; i++) seningUart.putDataByte((uint8_t)(i+S3pCode::MARK-2)); // Test stuffing
}
void sendTextMsgs() {
    seningUart.sendMsg(29, (const uint8_t*)"hello world how are you doing");
    seningUart.sendMsg(15, (const uint8_t*)"I am doing well");
    seningUart.sendMsg(16, (const uint8_t*)"and how are you?");
}
void sendBinaryMsg(int len) {
    seningUart.putCommand(S3pCode::BOM);
    sendBytes(len);
    seningUart.putCommand(S3pCode::EOM);
}
...
class S3pTestApplicationSide : public StaticThread<> {
    ...
    void run() {
        ...
        sendProbeMsgs();
        for(int i = 0; i < 4; i++) {
            len = receivingUart.getMsg(100, inputMessage);
