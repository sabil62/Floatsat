...
void udpReader([[gnu::unused]] int a) {
...
void UDPReceiver::reopen(const int port) {
    ...
    if(enableMultiReader) {
        ...
        // On macOS we have to use SO_REUSEPORT.
        if(setsockopt(sock, SOL_SOCKET, SO_REUSEPORT, &on, sizeof(on)) == -1) {
            xprintf("setsockopt SO_REUSEPORT failed\n");
        }

        // Set up Multicast.
        struct ip_mreq mreq;
        mreq.imr_interface.s_addr = htonl(INADDR_ANY);
        mreq.imr_multiaddr.s_addr = inet_addr(IP_BROADCAST_ADR);
        if(setsockopt(sock, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, sizeof(mreq)) == -1) {
            xprintf("setsockopt IP_ADD_MEMBERSHIP failed\n");
        }
    }
...
void dummy_handler([[gnu::unused]] int a) {
    xprintf("dummy_handler: should not reach here.\n");
